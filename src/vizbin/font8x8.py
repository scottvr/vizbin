"""Vendored 8x8 monochrome bitmap font (Basic Latin, U+0000..U+007F).

This is *data*, not a dependency: the table is embedded so the ``text`` render
mode can draw a readable glyph per byte while vizbin stays pure-stdlib.

Source:  font8x8 by Daniel Hepper <daniel@hepper.net>
         https://github.com/dhepper/font8x8  (font8x8_basic.h)
         Derived from Marcel Sondaar / IBM public-domain VGA fonts.
License: Public Domain (CC0). No attribution required; kept here as courtesy.

Layout:  ``FONT8X8[codepoint]`` is a tuple of 8 row bytes, top row first.
         Within a row byte, bit 0 (LSB) is the LEFTMOST column, bit 7 the
         rightmost -- i.e. pixel (col, row) is set iff ``rowbyte & (1 << col)``.
"""

from __future__ import annotations

FONT8X8: tuple[tuple[int, ...], ...] = (
